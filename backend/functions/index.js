...
exports.submitToAttack = httpsOnCall(async (req) => {
  ...
  if (pending.type === "Computer Virus") {
    const myApps = appPile.filter((c) => c.owner === uid && c.type === "app");
    if (myApps.length > 0) {
      // Select one random app card
      const idx = Math.floor(Math.random() * myApps.length);
      const selectedCard = myApps[idx];

      // Update the appPile to remove the selected card and return it to the deck
      appPile = appPile.filter((c) => c.id !== selectedCard.id);
      selectedCard.owner = null; // Remove ownership
      appDeck.push(selectedCard); // Return to deck
      appDeck = shuffle(appDeck); // Shuffle the deck
  ...
  if (pending.type === "Hacker Theft") {
    ...
    if (myApps.length > 0) {
      ...
      // Update the appPile to transfer ownership of the selected card
      appPile = appPile.map((c) => {
        if (c.id === selectedCard.id) {
          return { ...c, owner: pending.from }; // Transfer ownership
        }
        return c;
      });
